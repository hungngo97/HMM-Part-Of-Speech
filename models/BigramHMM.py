...
class HMM:
    ...
    def q(self, current_tag, previous_tag):
    ...
    def add_word_tag_value_backtrace(self, word, word_index, max_prod, candidate_previous_tag,
                                     current_tag):
        ...
        self.emission_transition[word_index][current_tag] = [
            max_prod, candidate_previous_tag]

    """
      Returns the corresponding value of the proposed tag in previous index
    """

    def previous_max_tag_value_backtrace(self, word_index, tag):
        ...
        elif index_value in self.emission_transition.keys() and tag in self.emission_transition[index_value].keys():
            value = self.emission_transition[index_value][tag][0]
        return value
    ...
    def set_begin_sentence(self, word):
        candidate_previous_tag = "*"  # TODO: Replace this with other START symbhol
        max_value = -1
        for tag in self.tag_unigrams.keys():
            if tag != '*':
                value = self.e(word, tag) * self.q(tag, candidate_previous_tag)
    ...
    def find_tag_sentence(self, words):
        ...
        for tag in self.emission_transition[i].keys():
            ...
            tag_value = self.emission_transition[i][tag][0]
            previous_tag = self.emission_transition[i][tag][1]
        ...
        while i > 0:
            previous_tag = self.emission_transition[i][current_tag][1]
            word_tags[i - 1] = previous_tag
            i -= 1
